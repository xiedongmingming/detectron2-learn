...
def patch_instances(fields):
    ...
    with tempfile.TemporaryDirectory(prefix="detectron2") as dir, tempfile.NamedTemporaryFile(
        ...
        try:
            # Objects that use Instances should not reuse previously-compiled results in cache, because `Instances` could be a new class each time.
            _clear_jit_cache()
...
def freeze_training_mode(model):
    ...
    # __constants__ is the old way to annotate constants and not compatible with __annotations__ .
